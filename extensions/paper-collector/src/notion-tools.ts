...
async function validateImageUrls(blocks: Array<Record<string, unknown>>): Promise<{
...
export function createNotionSavePaperTool(params: {
  ...
  return {
    ...
    execute: async (_toolCallId: string, rawParams: Record<string, unknown>) => {
      ...
      const dbId =
        (typeof rawParams.database_id === "string" && rawParams.database_id) || config.databaseId;
      if (!dbId) {
        return {
          content: [
            {
      ...
      const paper: PaperMetadata = {
        ...
        institutions: Array.isArray(rawParams.institutions)
          ? rawParams.institutions.map(String)
          : [],
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to save paper to Notion: ${msg}` }],
        };
      }
    },
  };
    ...
    papers: {
      ...
      items: {
        ...
        properties: {
          ...
          contributions: {
            type: "string" as const,
            description: "Main contributions (2-3 sentences).",
          },
          tags: {
            type: "array" as const,
            items: { type: "string" as const },
          ...
          conference: {
            type: "string" as const,
            description: "Venue with year (e.g., 'ICLR 2025').",
          },
          status: {
            type: "string" as const,
            enum: ["Unread", "Reading", "Read"],
  ...
  return {
    ...
    execute: async (_toolCallId: string, rawParams: Record<string, unknown>) => {
      ...
      if (!Array.isArray(rawPapers) || rawPapers.length === 0) {
        return {
          content: [{ type: "text" as const, text: "Error: papers array is empty or invalid." }],
        };
      }
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Batch save failed: ${msg}` }],
        };
      }
    },
  };
...
const ExtractPageImagesSchema = {
  ...
  properties: {
    html: {
      type: "string" as const,
      description: "Raw HTML content from web_fetch. The tool extracts image URLs from it.",
    },
    base_url: {
      type: "string" as const,
...
export function createExtractPageImagesTool(params: {
  ...
  return {
    ...
    execute: async (_toolCallId: string, rawParams: Record<string, unknown>) => {
      ...
      const baseUrl = typeof rawParams.base_url === "string" ? rawParams.base_url : undefined;
...
export function createExtractPaperFiguresTool(params: {
  ...
  return {
    ...
    execute: async (_toolCallId: string, rawParams: Record<string, unknown>) => {
      ...
      if (!arxivId) {
        return {
          content: [{ type: "text" as const, text: "Error: arxiv_id is required." }],
        };
      }
      ...
      for (const src of sources) {
        try {
          ...
          const resp = await fetch(src.url, {
            ...
            headers: {
              "User-Agent": "Mozilla/5.0 (compatible; PaperCollector/1.0; +https://openclaw.ai)",
            },
          });
          if (!resp.ok) {
          ...
          logger.info(`${src.name}: extracted ${images.length} candidate images`);
      ...
      if (allFigures.length === 0) {
        ...
        logger.warn(`No valid figures found for ${arxivId}. Errors: ${errors.join("; ")}`);
        return {
          content: [
            {
              type: "text" as const,
      ...
      const lines = allFigures.map((fig, i) => {
        const sizeStr = fig.bytes > 0 ? ` (${Math.round(fig.bytes / 1024)}KB)` : "";
        const altStr = fig.alt ? `\n   Caption: ${fig.alt}` : "";
        return `${i + 1}. ![Figure ${i + 1}${fig.alt ? `: ${fig.alt}` : ""}](${fig.url})${sizeStr}${altStr}\n   Source: ${fig.source}`;
      });

      logger.info(`Found ${allFigures.length} valid figures for ${arxivId}`);
...
const NotionWritePageSchema = {
  ...
  properties: {
    page_id: {
      type: "string" as const,
      description: "Notion page ID to write content to (the 32-char hex ID from the page URL).",
    },
    markdown: {
      type: "string" as const,
      description:
  ...
  return {
    ...
    execute: async (_toolCallId: string, rawParams: Record<string, unknown>) => {
      ...
      try {
        ...
        const imageCount = blocks.filter((b: Record<string, unknown>) => b.type === "image").length;
        const MIN_BLOCKS = 40;
        ...
        if (brokenImages.length > 0) {
          const brokenList = brokenImages.map((b) => `  - ${b.url} → ${b.status}`).join("\n");
          logger.warn(
        ...
        if (tooSmall.length > 0) {
          const smallList = tooSmall.map((s) => `  - ${s.url} → ${s.bytes} bytes`).join("\n");
          logger.warn(
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to write page content: ${msg}` }],
        };
      }
    },
  };
...
const NotionCreateChildPageSchema = {
  ...
  properties: {
    ...
    markdown: {
      type: "string" as const,
      description: "Page content in Notion-flavored Markdown (same format as notion_write_page).",
    },
  },
};
  ...
  return {
    ...
    execute: async (_toolCallId: string, rawParams: Record<string, unknown>) => {
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Failed to create child page: ${msg}` }],
        };
      }
    },
  };
